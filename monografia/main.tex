...
Por exemplo, ao criptografar e enviar um arquivo usando identidade de Bob concatenada com ano atual (``ano-2020''), pode-se conceber um cenário onde Bob devesse solicitar ao gerador de chaves a chave privada correspondente à chave pública ``Bob@mail.com ano-2020'', levando a requisições periódicas de novas chaves em anos subsequentes e implementando uma forma efetiva e automática de revocação de acesso de usuários simplesmente ao deixar de conceder novas chaves.
Adicionando tags do tipo ``acesso=restrito'' ao exemplo anterior pode-se pensar em um cenário onde o gerador de chaves iria exigir privilégios de acesso ou o aceite de termos de compromisso para conceder a chave privada relacionada a uma chave pública do tipo ``Bob@mail.com ano-2020 acesso=restrito''.
...
As políticas de acesso são convertidas em um esquema de compartilhamento de segredo \emph{(em inglês, SSS - Secret Sharing Scheme)}, onde os atributos ocupam o papel de participantes do esquema e o segredo é um parâmetro necessário ao algoritmo para poder descobrir a chave aleatória usada para criptografar um conteúdo.
...
Shamir demonstrou como produzir estruturas de acesso com uma porta de limiar $(k,n)$, permitindo a reconstrução do segredo se $k$ participantes se unirem, impedindo a reconstrução em número menor do que $k$ ou qualquer ganho de informação sobre o segredo \cite{Shamir1979}.
...
O esquema ABE em sua inicialização monta um esquema de compartilhamento de segredos compatível com o universo de atributos a serem utilizados, esquema este usado posteriormente para esconder um parâmetro cuja falta impede a descriptografia e que só pode ser recuperado ao apresentar as chaves dos atributos que correspondam à política de acesso (fórmula) delineada na estrutura de acesso.
Há duas formas básicas de estruturar a ABE, dependendo da parte em que a estrutura de acesso é alojada. A primeira forma de ABE gerava as chaves privadas baseadas em políticas que continham atributos, e por inserir a política de acesso na chave foi denominada como KPABE \emph{Key-Policy Attribute-Based Encryption}.
