...
O Web3j instancia um objeto singleton
...
Uma classe específica dentro do módulo cliente denominada \emph{BlockchainConnection} lida com a interface com a Blockchain.
A biblioteca Web3j possibilita a conexão, e o principal parâmetro para funcionar é um endereço válido apontando para uma porta socket de um processo executado no computador, um IP na rede local ou mesmo um serviço Web na internet que provenha o acesso à Blockchain.
Este parâmetro é passado durante a inicialização da classe e é armazenado no cache do programa em futuras conexões.
