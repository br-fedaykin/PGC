...
A fim de impedir ambiguidades, a chave pública é considerada a concatenação de seu nome e valor, possibilitando a diferenciação entre dois atributos de mesmo nome gerados por entidades distintas, impedindo também ataques do tipo \emph{spoofing}\footnote{Em um ataque de \emph{Spoofing} um atacante se passa por um terceiro falsificando informações com a finalidade de obter vantagens ilegítimas no sistema}.
...
O GID é um parâmetro adicional usado durante o processo de verificação da satisfatibilidade da estrutura de acesso no mecanismo de descriptografia e assegura o fracasso do procedimento quando atributos emitidos com GID distintos forem utilizados em conjunto para satisfazer uma política de acesso.
...
Isso permitiria que hospitais criassem seu próprio esquema ABE com sua própria criação e concessão de atributos.
...
Neste cenário, suponha que um centro de emergência receba um caso de um paciente em estado grave que já foi identificado por seus documentos e que está inconsciente. % TODO: (b) 2 suponhas nesse parágrafo
...
Este centro médico negociou o recebimento de uma chave pessoal deste atributo ``\emph{emergência}'', emitida pela autoridade certificadora de cada hospital que utiliza DCPABE e desta maneira ganha o acesso ao prontuário.
...
A chave privada associada ao endereço de carteira Ethereum é utilizada como valor do GID na geração das chaves públicas e privadas dos atributos no esquema ABE.
