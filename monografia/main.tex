...
Em sua primeira implementação no Bitcoin, a estrutura de um bloco é simplesmente um cabeçalho e um corpo com uma lista de transações.
O cabeçalho armazena informações relevantes para a verificação de sua integridade, informando dados como o hashing do bloco anterior, da árvore Merkle das transações do bloco, um hashing do próprio cabeçalho, e metadados da rede como versão do protocolo e parâmetros para o algoritmo de consenso.
A Figura \ref{fig:blockchain} demonstra essa estrutura.

\begin{figure}[htp]
    \centering
    \includegraphics[width=\textwidth]{images/exemplo-de-blockchain.png}
    \caption{Estrutura de uma Blockchain}
...
Ao invés de implementar uma lista preemptiva e exaustiva de todas as transações que pudessem vir a ser necessárias, o protocolo do Bitcoin imbui as transações com uma linguagem de script baseada em pilha, intencionalmente turing-incompleta, sem loopings ou recursividade, contendo algumas operações aritméticas, lógicas e criptográficas \cite{Narayanan2016a}.

As transações utilizam essa linguagem de script para definir o comportamento de suas entradas e saídas.
Uma saída especifica uma quantidade de bitcoins e usa um script para trancar este valor sob um desafio criptográfico, que poderá ser utilizado por uma transação posterior que a referencie em uma de suas entradas. Uma entrada contém uma referência à uma transação e à posição da lista de saídas que pretende gastar, com o script que resolve o desafio relacionado àquela saída.
A checagem de uma transação se dá pela união dos scripts de entrada e saída e na sua execução pelo cliente do Bitcoin e é considerada válida se, e somente se, para cada uma de suas entradas, a união dos scripts resultar em um valor verdade na pilha ao fim de sua execução.

Diversos comportamentos foram então modelados em scripts  creditados como seguros, a exemplo da transferência mais comum, que é a transferência de moeda entre carteiras, implementada pelo script conhecido como P2PKH \emph{(Pay to Public Key Hash)}, que condiciona a utilização de um certo valor destinado a uma chave pública mediante a assinatura produzida pela chave privada relativa àquela mesma chave. Outros scripts juntamente com este formam um pequeno conjunto de scripts alçados ao status de padrão no protocolo do Bitcoin. Uma transação é considerada padrão quando só utiliza scripts padrões, e mais de 99,9\% das transações na Blockchain são padrões \cite{Bistarelli2019}. A alta adesão não é coincidência, pois o cliente oficial do Bitcoin não permite a propagação de transações não-padrão pela
...
% rascunhos do capítulo
\begin{comment}
Outras possibilidades eram visadas além da transferência de fundos. Multisig, arbitragem, pagamentos condicionados a chaves e a tempo já expandiam as possibilidades do bitcoin.

Possibilidade de salvar dados na blockchain abriu margem para visualizar o conceito de Distributed Ledger --- bases de dados imutáveis.

A comunidade do Bitcoin vê com maus olhos o desvio do propósito do btc como plataforma de pagamentos.

A Blockchain 2.0 visa dar suporte ao conceito de Distributed Ledger, servindo para registrar dados na rede, e também o conceito de smart contracts, para flexibilizar o estado de transação da moeda.

Embora a rede Bitcoin fosse modelada desde seu princípio como uma criptomoeda, ela também demonstrou ser uma nova forma de ser uma base de dados distribuída, já que dados arbitrários poderiam ser gravados em suas transações.

A Blockchain é uma abstração adequada para o conceito de livro-razão, uma estrutura de dados comum na área contábil, utilizada para lançamento de todas as movimentações contábeis de uma empresa. O livro-razão

O modelo de dados de uma criptomoeda pode ser abstraído como se fosse um \emph{Distributed Ledger} \emph{(Livro-razão distribuído)}, uma estrutura de dados com função similar ao registro contábil ou
\end{comment}


% detalhamento do Bitcoin que o professor disse estar fora de escopo
\begin{comment}
