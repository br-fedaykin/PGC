...
% \hyphenation{0x584cf84bcc\-601da596b4f12a4c976577b40e9970}
...
Na área da saúde, o compartilhamento de dados médicos permite a melhoria na qualidade do atendimento e a redução de custos de operação, entretanto, obstáculos técnicos têm impedido a comunicação dos sistemas de saúde, envolvendo questões como privacidade, segurança, integridade e interoperabilidade destes sistemas.
...
Analisamos os limites de compatibilidade na codificação das estruturas de dados da CBA em uma transação Ethereum e implementamos um protótipo para verificar a viabilidade da implantação e operação do sistema.

\noindent \textbf{Palavras-Chave:} Blockchain; Ethereum; Contratos Inteligentes; CBA; RES.
\end{abstract}
...
In the Healthcare sector, data sharing improves the quality of medical care and reduces operation costs, but technical problems impose difficulties in health systems communication, problems related to privacy, security, integrity, and interoperability.
Nowadays, the institution that produced the medical data usually is the one that controls the access of it, when in fact the control should be exercised by the patient, the owner of such data.
Besides, the patient cannot control or consent upon the access conditions to his personal data, sometimes he has no access to it, which is made available only upon request.
In this work, we propose a distributed, Blockchain-based system for sharing and access control of Health Records.
The control is patient-centric, as he is responsible for the access policies, described in terms of attributes generated by institutions that play the role of authorities in an attribute-based Cryptography (ABE) scheme.
Users and institutions use Smart Contracts on an Ethereum network to interact and query about record metadata and attributes published by certifiers.
We analyze the compatibility limits in the codification of ABE data structures in an Ethereum transaction and we implement a prototype to verify the viability of system deployment and operation.

\noindent \textbf{Keywords:} Blockchain; Ethereum; Smart Contracts; ABE; EHR.
\end{abstract}
...
Para isso, o esquema propõe uma plataforma multi-autoridades, cada uma delas com um esquema ABE próprio, que pode ser criado e posto em operação sem necessidade de qualquer coordenação entre as autoridades \cite{Lewko2011}.% além do uso de um mesmo conjunto de parâmetros iniciais de configuração
...
%As políticas de acesso foram aplicadas pelo próprio usuário, mas poderia também ter sido aplicada pelo hospital com o aval do paciente nos casos em que há dificuldade.
...
%Este cenário privilegia o atendimento de emergência ao considerar a presença prévia do atributo ``\emph{emergência}'' nas políticas de acesso, mas isto não constitui uma obrigatoriedade. Sem este atributo em um prontuário, este seria aplicado sob demanda, quando por exemplo um hospital recebesse um pedido de acesso ao prontuário junto a uma notificação de internação do paciente, sendo removido das condições de acesso quando fosse possível delegar a decisão ao paciente ou seu representante legal.
...
%Uma transação é considerada padrão quando só utiliza scripts padrões, e mais de 99,9\% das transações na Blockchain são padrões \cite{Bistarelli2019}. A alta adesão não é coincidência, pois o cliente oficial do Bitcoin não permite a propagação de transações não-padrão pela rede \footnote{Não é relevante descrever aqui todas as restrições e validações de transações. É possível ver a lista completa na página da \link{https://en.bitcoin.it/wiki/Protocol_rules\#.22tx.22_messages}{Bitcoin Wiki}}, virtualmente negando o serviço a transações deste tipo, impondo ao seu proponente o ônus de propagar sua transação de forma externa à rede na tentativa de fazê-la alcançar os nós que tenham a eventual chance de incluir um bloco na Blockchain.
...
%A distribuição do novo bloco e sua referência em novas propostas de blocos são o consenso da rede de que ele realmente participa da Blockchain. Por isso pode-se dizer que o consenso ocorre \textbf{implicitamente}, ou seja, não há coordenação de mensagens para desencadear um processo de decisão formal para que um determinado bloco seja aceito ou rejeitado. Ao invés disso, o consenso se dá pela inclusão, por meio de sua referência, na cadeia de blocos.
...
Geralmente, isto se resolve guardando no contrato uma lista de endereços permitidos %ou associando-se os dados inseridos no contrato com os endereços de origem,
...
%Isso ocorre porque parte da lógica das políticas de acesso considera que o próprio cliente (\emph{i.e.}, o certificador) possa criar os próprios atributos e concedê-los a terceiros (outros clientes).
...
Os contratos também possuem funções de pesquisa e recuperação de dados  utilizados em determinadas operações pelo módulo Cliente. % para manter a consistência de seu cache local.
...
\begin{itemize}
  ...
  \item O módulo cliente faz a publicação do endereço da etapa anterior na Blockchain junto aos metadados do arquivo para indicar que o endereço estará associado a ele pela publicação; %será realmente utilizado pelo usuário para enviar o documento (que passa a estar associado a ele pela publicação);
...
As razões para isso são para conservar o escopo deste trabalho e para ater-se ao cenário de uso multi-institucional, que pressupõe uma diversidade de políticas de segurança e privacidade. %, possivelmente incompatíveis entre si.
...
\begin{figure}[H]
  \centering
  \includesvg[width=1\linewidth]{images/diagrama-SmartDCPABE.svg}
...
\begin{enumerate}[label=(\arabic*)]
  ...
  o primeiro é o próprio RES, cifrado com AES; o segundo contém a chave utilizada na criptografia AES, que está guardada sob uma cifra ABE definida na política associada ao RES;%, tarefa que pode ser automatizada no caso de uma migração de bases de dados para o sistema proposto;
...
Embora todo usuário crie e use um atributo com o mesmo nome, não existe conflito entre eles porque um atributo é a conjunção de um rótulo e uma chave pública. Nesse sentido, a descriptografia só é possível mediante a posse de uma chave pessoal de atributo (emitida pelo dono da chave privada) usada para a derivação da chave pública gravada no atributo. %Assim, somente o emissor do atributo $paciente$ tem as credenciais compatíveis para satisfazer a política de acesso onde o atributo for usado.
...
Após a obtenção do endereço, o módulo Cliente envia à Blockchain diversos dados sobre o documento: a identificação do servidor, o endereço único naquele servidor, nome do arquivo, política de acesso,  entre outros. %o instante em que a transação está sendo feita e dados produzidos pela criptografia ABE (necessários no processo de descriptografia do arquivo).
...
%Alice então envia o documento para o servidor de arquivos do Hospital Mário Covas e publica informações sobre o arquivo na Blockchain, entre elas a política de acesso para poder acessar o documento.
...
%Em suma, caberiam às instituições decidirem arbitrariamente com quais atributos deveriam trabalhar. Diferenças quanto às suas competências, especialidades, políticas próprias de acesso à informação e outras normas internas certamente poderiam implicar em arranjos de atributos conflitantes, ao invés de complementares.
...
Para estabelecer este padrão, pode-se aproveitar os conteúdos dispostos nas normas que padronizam os Sistemas de Registro Eletrônico de Saúde (S-RES). %, a forma como são denominados na lei os sistemas de informação na área da saúde.
...
Ainda que a taxonomia de atributos colabore para a interoperabilidade entre as instituições, não resolve de todo o problema do esquema ABE, porque as próprias políticas de acesso carecem de padronização. %, abrindo margem para a aplicação de condições de acesso arbitrárias, imprevisíveis ou mesmo ilegais.
Nesse sentido, sem uma normalização das políticas de acesso aos RES, a produtividade dos usuários que operam o sistema será prejudicada. Isto devido à heterogeneidade das condições de acesso entre instituições, %, e os custos de automação do sistema serão afetados e aumentarão,
...
Por conta disto, além da taxonomia, foi desenvolvido um diagrama de condições de acesso ao RES a partir do Código de Ética Médica (CEM) \cite{ConselhoFederaldeMedicina2019}, onde se descrevem várias regras a serem cumpridas por médicos e instituições, % onde prestam serviço em vários aspectos,
...
O CEM também distingue a natureza de certas informações dentro do prontuários, aplicando regras diferentes a certos conteúdos, entretanto sem apresentar um modelo formal da segmentação dos mesmos. %ou explicitar alguma sistematização quanto a essas partes que compõem o prontuário.
Por conta disso, foi realizada a segmentação do prontuário mostrada na Tabela \ref{tbl:segmentacoes-prontuario}, que lista os segmentos observados a partir da diferenciação feita pelas regras de acesso aos conteúdos. %, evidenciando naturezas diferenciadas das informações.
...
%Todas as outras regras definidas pelo CEM são transitórias (como é o caso de um Perito ou Pesquisador), dependentes de algum acontecimento (como é o caso de terceiros que precisam aguardar a autorização do paciente), ou somente se aplicar a uma parte do prontuário (como é o caso de seguradoras e de familiares, que por lei só teriam direito a acessar o atestado e o laudo de óbito)~\footnote{Embora a lei não preveja o acesso irrestrito dos familiares ao prontuário do familiar falecido, o diagrama considera esta possibilidade devido a notícias de decisões judiciais em favor disso \cite{G1SE2013}.}.
...
Finalmente, é possível que um usuário entregue acesso a terceiros ao emitir o atributo \textit{Terceiro-Autorizado} para eles. Para isso, o usuário poderia aplicar este atributo nos mesmos lugares onde aplicou o atributo $Paciente$, de forma a garantir o acesso completo ou parcial ao prontuário. %, ou aplicar nos mesmos moldes explicados na situação de acesso restrito, onde ele poderia escolher quais partes do prontuário devem ser disponibilizadas aos terceiros.
...
Já os certificadores são os atores do sistema responsáveis pela criação, publicação e concessão de atributos aos usuários que necessitem deles. % (e tenham, claro, o direito de solicitá-los).
...
Assim, o usuário precisa publicar sua requisição de atributos (de um mesmo certificador) em um Smart Contract específico para a gestão de requisições, catalogada inicialmente como ``pendente''. %até que o certificador atualize a situação do pedido.

Em algum momento, o certificador obterá as requisições pendentes e poderá alterar suas situações. % de requisições quando pendentes, efetivamente consumindo a requisição no processo de atualização da situação.
...
%eliminando desta forma a existência de outros algoritmos e chaves que não fossem relacionados ao trabalho.
...
Entre essas fontes, podem-se citar a rede principal (\emph{i.e.}, \emph{MainNet}), as redes de teste oficiais (\emph{i.e.}, Ropsten, Rinkeby e Kovan) ou simuladores locais da rede Ethereum. % para teste, por meio de programas como o Ganache~\cite{Ganache2020}.
...
Dentro desse módulo, a classe \emph{BlockchainConnection} é a responsável por lidar com a Blockchain (por meio do Web3j) e com as classes em Java que representam os Smart Contracts. Como mostra o código \ref{cod:conexaoBlockchain}, para realizar o acesso  somente é exigido um endereço IP/porta válido (atributo networkURL da linha 5) no computador onde exista comunicação com a API JSON-RPC do Ethereum.%, ou com um serviço Web que ofereça essa API, tais como os sites da Infura e do Ethercluster.
...
Cabe destacar que não seria complicado implementá-los no Smart Contract, porém, não seria fácil implementar os protocolos de revogação e substituição de atributos.% no módulo Cliente, responsável pelas operações criptográficas e pela coordenação das informações entre a Blockchain e o Servidor.
...
O mapa \emph{pendingRequesters} (linha 19) mantém os endereços de usuários com requisições pendentes em listas agregadas por certificador. %, possibilitando que estes possam iterar pelo mapa \emph{pendingRequests}, obtendo os índices de acesso das requisições de interesse gravadas em \emph{requests}.
...
O design de Smart Contracts e testes de integração exigem essa dinâmica, %e pode-se querer ainda verificar o comportamento da aplicação após sua implantação,
...
Há todo um ecossistema em torno da rede principal Ethereum que permite a simulação da rede. %em graus diferentes de similaridade.
...
%Embora baste que um desenvolvedor importe a biblioteca Web3.js em um código Javascript para que o site tenha a capacidade de se conectar com processos Ethereum, a forma mais comum, mais segura e recomendada de integrar um serviço Web com Ethereum é por meio do Metamask.
...
\begin{figure}[!h]
  \centering
  \includesvg[width=450pt]{images/infraestrutura-Blockchain.svg}
...
%O Ganache faz parte de uma plataforma de desenvolvimento de Blockchains em Javascript denominada Truffle.
...
Foi necessário a passagem do tempo para que se tornasse evidente a viabilidade da inovação trazida com o Bitcoin, atraindo a atenção do meio acadêmico e industrial. % que passou a estudar suas propriedades, alternativas e aplicações.
...
Os autores Yang e Yang estendem a funcionalidade do MedRec para prover tanto a confidencialidade dos dados quanto uma forma anônima de autenticação, baseada em atributos. %, porém rastreável se necessário, .
...
%Eles usam um cliente Bitcoin modificado para conter transações que representem registros de prontuários médicos com substituição do mecanismo de geração de chaves e assinatura nativo do Bitcoin por outro baseado em um esquema ABE descentralizado que permite que múltiplas autoridades criem e concedam atributos a usuários.
...
Akinyele et al.~\cite{Akinyele2010} usam ABE para implementar o conceito de prontuários médicos autoprotegidos, isto é, prontuários cujos dados estão intrinsecamente seguros contra acesso não autorizado. Esses prontuários carregam as informações necessárias para permitir o acesso baseado em restrições.% e limitações embutidas no arquivo.
...
Nesta seção mostram-se os resultados experimentais colhidos a partir do sistema descrito na seção \ref{sec:sistema-proposto}.
O objetivo principal é analisar a viabilidade da codificação de um esquema ABE na Blockchain.
Para atingir este objetivo, procedeu-se a análise de dois aspectos que compõem um esquema ABE: a cifra e a concessão de um atributo a um usuário por um certificador, analisado nas subseções \ref{sec:sub:experimento-cifra} e \ref
...
Os testes foram realizados em um PC com sistema operacional Windows 10 x64, 16GB de memória RAM e processador intel i5-4670 @ 3.40 GHz. Dentro os aplicativos foram utilizados: Java versão 14.0.2; Solidity Compiler 0.7.5+commit.eb77ed08; Web3j 4.8.0 e a versão \emph{istanbul} da EVM Ethereum.
...
Com exceção do vetor c0, os elementos restantes (c1, c2 e c3) são vetores de vetores e variam em tamanho dependendo da quantidade de atributos utilizados na política de acesso.
...
A variável de tamanho dinâmico dessa estrutura de dados impõe à requisição as mesmas limitações discutidas na seção anterior (que trata sobre a codificação da cifra), limitando a quantidade de atributos que podem ser solicitados em uma mesma requisição.% por causa das limitações decorrentes do limite de gás em blocos e, por extensão, às transações.
