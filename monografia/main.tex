...
\begin{enumerate}
  ...
  \begin{enumerate}[label*=\arabic*.]
    \item O limite de uso de gas em um bloco acabam impondo limites ao tamanho das transações e, por decorrência, ao tamanho da Cifra.
    \begin{enumerate}[label*=\arabic*.]
      \item Podemos calcular o tamanho máximo da cifra de acordo com o limite de gás imposto ao bloco.
      \begin{enumerate}[label*=\arabic*.]
        \item O limite máximo da cifra mensurado em testes será inferior ao calculado pelo limite de gás, porque a conta não considera o custo de gas das outras instruções executadas pela função que armazena a cifra ou pela função que recupera a cifra.
      \end{enumerate}
  ...
  \begin{enumerate}[label*=\arabic*.]
    \item chaves públicas de atributos são constituídas por um par com nome e um valor de chave.
    ...
    \item A publicação de um atributo tem um custo constante devido ao tamanho aproximadamente constante que seus elementos constituintes assumem.
  \end{enumerate}
  ...
  \begin{enumerate}[label*=\arabic*.]
    \item É possível calcular quantas requisições podem ser solicitadas de forma teórica, da mesma forma que foi feita com a cifra. O número será muito superior à quantidade possível de especializações que um ser humano poderia obter ao longo de sua vida.
    ...
    \item o custo em gas da requisição por atributos vai crescer de forma não-linear (mais que linear) devido ao campo de tamanho dinâmico que contém a lista de nomes de atributos requeridos.
  \end{enumerate}
  \begin{enumerate}[label*=\arabic*.]
    \item os custos associados à operação do sistema e aquisição de Ether tornam viável a implantação de uma rede privada
    \begin{enumerate}[label*=\arabic*.]
      \item as operações relacionadas à cifra são as que podem custar mais gas e as que vão incorrer em mais custo para os usuários.
      \begin{enumerate}[label*=\arabic*.]
        \item alterar as condições de acesso do prontuário significa republicar uma nova cifra para o prontuário em questão. A alteração será necessária a cada novo ciclo de atendimentos por uma especialidade de um hospital, impondo custos proporcionais à frequência de encaminhamento entre hospitais e especialidades.
        \item a aplicação das políticas descritas na seção \ref{sec:sub:padronizacao-permissoes} impõem, na prática, um custo mínimo associado à publicação de um arquivo.
      \end{enumerate}
      \item a flutuação da cotação do Ether dificuldade a previsibilidade do custo de operação do sistema
...
\begin{enumerate}
  ...
  \item Atributos podem ser colocadas dentro de transações na Blockchain sem limitações.
  \begin{enumerate}[label*=\arabic*.]
    \item Exibir um exemplo de chave pública de atributo, contendo nome e valor da chave.
    \item fazer uma tabela de geração de atributos para demonstrar que os dois vetores de bytes que compõem o valor de uma chave pública apresentam um tamanho variável, mas que variam em um intervalo específico, entre 125 e 127 bytes. Informar que a observação de limite inferior e superior para o valor da chave de um atributo motivou a criação de uma estrutura denominada ``bytes127'' no SmartDCPABEKeys, que é utilizada no lugar de uma variável do tipo ``bytes'', que é de tamanho variável e poderia trazer custos maiores na publicação de atributos.
    \item fazer um gráfico com a distribuição das transações de publicação de atributos na blockchain e mostrar que há uma concentração entre. Informar que a amostra de atributos foi gerada com a mesma semente global de inicialização do DCPABE, mas com um conjunto aleatório de nomes e de GID de autoridades (esses são os 3 elementos que influenciam a derivação da chave).
  \end{enumerate}
  \item Requisições por atributos também dependem de uma variável com tamanho dinâmico, entretanto não apresentam a mesma limitação da cifra.
  \begin{enumerate}[label*=\arabic*.]
  ...
  \begin{enumerate}[label*=\arabic*.]
    ...
    \begin{enumerate}[label*=\arabic*.]
      ...
      \item não sei o que informar nessa hipótese pra validar a afirmação dela. Talvez eu precise remover essa parte do texto.
      ...
      \begin{enumerate}[label*=\arabic*.]
        \item Relacionar em uma fórmula todos os fatores que podem influenciar no custo (tanto em tempo, quanto em ether) para proceder a substituição de um atributo por outro.
        \item fazer um gráfico em barras com as atividades de um usuário (consultar usuário, consultar certificador, emitir atributo, publicar arquivo, etc) em uma aplicação de uso mínimo (descrever este ``uso mínimo'' como a criação de um atributo, criptografia e publicação de um arquivo da pessoa A, requisição do atributo de uma pessoa B e obtenção do arquivo publicado por A). Mensurar a diferença de execução dos contratos com e sem verificação e colocar essa diferença em um gráfico de barras (Obs: leva pouquíssimo tempo para adequar as funções a esse tipo de verificação, então é viável mensurar essas diferenças)
      \end{enumerate}
    \end{enumerate}
  \end{enumerate}
\end{enumerate}
