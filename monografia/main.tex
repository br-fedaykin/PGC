...
A proposta da DCPABE \emph{(Decentralized Ciphertext-Policy Attribute Based Encryption)} busca resolver esse problema ao permitir um ambiente ABE com multi-autoridades que podem ser criadas e operar sem necessidade de qualquer coordenação além do uso de um mesmo conjunto de parâmetros iniciais de configuração \cite{Lewko2011}.
A DCPABE permite que qualquer participante pode se tornar uma autoridade e criar atributos, gerando para cada atributo uma chave pública e uma chave privada, esta última usada para gerar chaves pessoais de usuário.
Esse sistema permite a escrita de políticas de acesso usando atributos emitidos por diferentes autoridades, tornando-se atrativo como a base fundacional de um sistema de integração criptograficamente seguro.
Para impedir ataques de colusão por parte dos usuários, a derivação das chaves é feita usando um identificador global de usuário.
Este identificador passa a fazer parte do mecanismo de descriptografia e assegura o fracasso da criptografia se atributos emitidos com diferentes identificadores globais forem utilizados para satisfazer uma política de acesso.
