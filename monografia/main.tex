...
Ela depende por sua vez da biblioteca \emph{jPBC} (Java Pairing Based Cryptography), a primeira e mais abrangente biblioteca de código aberto para criptografia baseada em emparelhamento em Java e compatível com Android, tendo o objetivo explícito de tanto portar a biblioteca PGC para código nativo em Java quanto de oferecer uma interface para essa biblioteca \cite{DeCaro2011}.
...
Ao criptografar um documento, os parâmetros necessários para a decifragem são serializados e prefixados no arquivo container dos bytes criptografados.
Esse comportamento foi alterado para produzir um artefato com os mesmos parâmetros que pode ser serializado em JSON,separado do arquivo que contem os bytes criptografados, com a finalidade de poder publicar na Blockchain somente este artefato JSON ao invés de todo o arquivo, tornando o sistema mais viável quanto aos custos de armazenamento.
...
\lstset{basicstyle=\small, numbers=left, language=java, keywordstyle=\color{blue}, numbersep=1pt, label={lst:conexãoBlockchain}, caption={Criptografando um arquivo usando a biblioteca DCPABE}}
