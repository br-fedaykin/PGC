...
O módulo do servidor conta com uma estrutura padrão de acordo com os princípios REST, ou seja, oferece uma API acessível (via protocolo HTTP) com métodos GET, POST e PUT, atribui URIs a todos os recursos sob gestão do servidor e pode executar com sucesso as requisições sem precisar gerenciar o estado de cada conexão realizada, i.e., possui um modelo de comunicação \emph{stateless}.
...
Um esquema inspirado no conceito da Prova de Conhecimento-Zero \emph{(Zero Knowledge-Proof - ZPK)}, onde um verificador (o servidor) pode se "convencer" que um provador (o cliente) realmente possui as credenciais necessárias para acessar o arquivo, sem com isso expor suas chaves ou ter que informar sua identidade para deixar a cargo do servidor a verificação na Blockchain dos atributos concedidos ao usuário e a compatibilidade destes com a política de acesso do arquivo.
Uma solução viável é fazer o servidor exigir que o cliente, ao enviar o arquivo a ser hospedado, produzir o hashing deste arquivo, cifrá-lo com a mesma política usada para o arquivo e enviar ambos.
O comportamento do servidor seria levemente modificado para que, diante de uma requisição de um arquivo sem parâmetros adicionais, a resposta seria um objeto cifrado contendo o valor do hashing do arquivo que se deseja requisitar.
Um usuário com as credenciais necessárias seria capaz de decifrar esse objeto, e realizar novamente a mesma requisição do arquivo, desta vez informando o hashing.
Diante de uma requisição de arquivo com algum parâmetro adicional, o servidor verifica se ele é equivalente ao valor do hashing do arquivo requisitado.
Em caso afirmativo, é evidente que o usuário tem consigo os atributos necessários para acessar o arquivo e o servidor envia o arquivo, negando o acesso no caso contrário.
