...
Invocações às funções de um contrato se dão por meio de uma transação, contendo argumentos codificados em bytes para sua execução e um limite computacional compulsório, expresso em termos de um recurso quantitativo denominado \emph{gas}.
O gas é uma unidade calculada a partir de dados de uso da rede e convertida automaticamente a partir do \emph{Ether}, a moeda base do Ethereum, e serve para lastrear o uso de computação da rede Ethereum em um recurso escasso e finito.
...
Essa mecânica incentiva um uso consciente da capacidade computacional da rede, contorna o problema da indecibilidade quanto ao fim da execução ou não de um programa, impedindo a parada da \emph{Ethereum Virtual Machine (EVM)} diante da execução de funções que rodariam indefinidamente e por fim serve para proteger os recursos do próprio usuário, diante de um custo imprevisto ou desconhecido de uma função em um Smart Contract com o qual pretende interagir, quer seja o custo inerente à função devido à complexidade da operação ou só produto de um código ineficiente ou mesmo incapaz de terminar.

{\color{ForestGreen}Explicar que a linguagem utilizada é denominada Solidity. Explicar o que é solidity. (1 parágrafo).}

A linguagem na qual Smart Contracts são publicados na rede Ethereum se chama \emph{Ethereum Virtual Machine code (EVM code)} --- uma linguagem de baixo nível baseada em pilhas, similar à linguagem \emph{Forth}. A própria fundação Ethereum desenvolveu linguagens de programação de mais alto nível, com sintaxes similares às linguagens mais utilizadas no mercado, com ferramentas que as compilem para o EVM code. O \emph{Solidity} é uma destas linguagens, com sintaxe similar a Java e C\texttt{++}, com compiladores e opções de compilação tais como otimização de código, além de um crescente ecossistema composto por padrões de programação, bibliotecas e ferramentas de desenvolvimento\footnote{Uma descrição do ecossistema Solidity em \href{https://github.com/bkrem/awesome-solidity}{https://github.com/bkrem/awesome-solidity}. As bibliotecas atualmente com maior base de usuário são, nesta ordem, \emph{\href{https://openzeppelin.com/}{OpenZeppelin}}, \emph{\href{https://github.com/dapphub/dappsys}{Dappsys}} e \emph{\href{https://github.com/modular-network/ethereum-libraries}{Modular Libraries}}}, destancando-se o ambiente de desenvolvimento e implantação de Smart Contracts \emph{Remix}, utilizada neste trabalho para codificar, depurar e implantar os Smart Contracts do protótipo em redes de teste Ethereum.
