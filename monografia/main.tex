...
% Hipótese Principal: É possível (i.e, viável) aplicar ABE a um sistema de distribuição de arquivos gerenciados por Smart Contracts.

% A hipótese principal pode ser destrinchada em uma série de hipóteses mais específicas:

% % HIPÓTESES
% \begin{enumerate}
%   \item Atributos podem ser colocadas dentro de transações na Blockchain sem limitações.
%   \begin{enumerate}[label*=\arabic*.]
%     \item isto porque chaves públicas de atributos são constituídas por um par com nome e um valor de chave.
%     \item Pela experimentação, verificou-se que o valor da chave pública de um atributo tem um tamanho entre 125 e 127 bytes, e portanto, é possível de ser representado em uma estrutura de tamanho fixo. O nome do atributo também foi restringido a um tamanho fixo.
%     \item Logo, a publicação de um atributo tem um custo constante devido ao tamanho aproximadamente constante que seus elementos constituintes assumem.
%   \end{enumerate}
%   \item Requisições por atributos também dependem de uma variável com tamanho dinâmico, entretanto não apresentam a mesma limitação da cifra.
%   \begin{enumerate}[label*=\arabic*.]
%     \item É possível calcular de forma teórica quantos atributos podem ser solicitados em uma mesma requisição, seguindo o exemplo de estimativa do tamanho da cifra. O número será muito superior à quantidade possível de especializações que um ser humano poderia obter ao longo de sua vida.
%     \item Mesmo que esse limite seja um problema, uma requisição com uma lista grande de atributos pode ser dividida em algumas requisições com subdivisões da lista de atributos.
%     \item É esperado que o custo em gas da requisição por atributos vai crescer de forma não-linear (mais que linear) devido à presença do campo de tamanho dinâmico que contém a lista de nomes de atributos requeridos.
%   \end{enumerate}
%   \item os custos associados à operação do sistema e aquisição de Ether tornam inviável o uso da rede principal e sugerem a implantação de uma rede privada
%     \begin{enumerate}[label*=\arabic*.]
%       \item isto porque as operações relacionadas à cifra são as que podem custar mais gas, as que vão incorrer em mais custo para os usuários e as mais frequentes.
%       \begin{enumerate}[label*=\arabic*.]
%         \item Isto porque alterar as condições de acesso do prontuário significa republicar uma nova cifra para o prontuário em questão. A alteração será necessária a cada novo ciclo de atendimentos por uma especialidade de um hospital, impondo custos proporcionais à frequência de encaminhamento entre hospitais e especialidades.
%         \item Lembrar também que a aplicação das políticas descritas na seção \ref{sec:sub:padronizacao-permissoes} impõem, na prática, um tamanho mínimo à cifra e, consequentemente, um custo mínimo para a publicação de um arquivo.
%       \end{enumerate}
%       \item a flutuação da cotação do Ether dificulta a previsibilidade do custo de operação do sistema
%       \item uma rede privada de nós compatíveis com o protocolo Ethereum ofereceria mais previsibilidade aos custos, que seriam basicamente equipamento, manutenção e energia.
%       \item a ausência de um mecanismo de revocação de chaves de atributos pessoais de usuários em ABE também implicam em formas ineficientes (em tempo de processamento e gas em transações) para impedir o acesso de um usuário removido.
%       \begin{enumerate}[label*=\arabic*.]
%         \item a solução mais efetiva (porém ineficiente) para remoção do acesso de um usuário consiste em identificar todos os atributos concedidos a ele ao longo do tempo, e promover uma substituição destes atributos por novos. A autoridade certificadora deveria emitir novos atributos pessoais para todos os demais usuários afetados pela substituição de chaves, e cada arquivo no sistema utilizando os atributos substituídos deveria ter sua cifra atualizada pela realização de nova criptografia utilizando o conjunto de atributos recém criados.
%         \item uma solução mais econômica (porém incompleta) para remoção do acesso de um usuário consiste em
%         criar uma ``lista negra'' para listar os usuários removidos da plataforma, entretanto esta solução permite que o usuário removido continue com um conjunto válido de atributos, de forma que ele possa continuar tendo acesso ilegítimo a arquivos mesmo após sua exclusão nos casos em que ele consegue realizar um cadastro de usuário falso, ou se ele operar em conluio com outro usuário válido no sistema.
%       \end{enumerate}
%   \end{enumerate}
% \end{enumerate}
% % FIM DAS HIPÓTESES

% Para testar a validade das hipóteses acima, será preciso recolher os dados abaixo e providenciar a seguintes análises:

% % DADOS E ANÁLISES
% \begin{enumerate}
%   \item Atributos podem ser colocadas dentro de transações na Blockchain sem limitações.
%   \begin{enumerate}[label*=\arabic*.]
%     \item Explicar que uma chave pública de atributo é feita por um par de nome e valor da chave, e exibir como exemplo os dados de um atributo gerado durante os testes.
%     \item fazer uma tabela (ou um gráfico de distribuição de probabilidade) para demonstrar que os dois campos em bytes que compõem o valor de uma chave pública de atributo apresentam um tamanho variável, mas que variam em um intervalo específico, entre 125 e 127 bytes. Explicar que a observação de limite inferior e superior para o valor da chave de um atributo motivou a criação de uma estrutura denominada como ``bytes127'' no SmartDCPABEKeys, que é utilizada no lugar de uma variável do tipo ``bytes'', que é de tamanho variável e poderia trazer custos maiores na publicação de atributos. Desta maneira este campo usa uma estrutura personalizada, definida usando somente dados de tamanho fixo, para evitar o uso de um tipo de dados de tamanho dinâmico, que é mais versátil, porém mais caro.
%     \item fazer uma tabela com desvio padrão (um gráfico de distribuição de probabilidade) de custo das transações de publicação de atributos na blockchain e mostrar que há, aproximadamente, um custo constante. Deixar claro que a amostra de atributos foi gerada com a mesma semente global de inicialização do DCPABE, mas com um conjunto aleatório de nomes e de GID de autoridades (esses são os 3 elementos que influenciam a derivação da chave).
%   \end{enumerate}
%   \item (vide subitens)
%   \begin{enumerate}[label*=\arabic*.]
%     \item fazer um raciocínio análogo aos itens \ref{enum:custo-teorico-de-gas-ciphertext} e \ref{enum:custo-de-gas-ciphertext}, aplicado à requisição de atributos. Se uma requisição de atributos permitir muitos atributos (mais de 80 por exemplo), argumentar que uma requisição tem capacidade suficiente para listar mais papéis do que um ser humano poderia ter.
%     \item Caso o cálculo da requisição aponte uma baixa capacidade quanto à lista de atributos, argumentar que o problema é automaticamente mitigado pela divisão de uma requisição com uma lista grande de atributos em um conjunto de requisições, cada uma com um subconjunto de atributos.
%     \item Fazer um gráfico demonstrando que o custo em gas da requisição cresce de forma não linear em relação ao número de atributos, devido, mais uma vez, à existência de um campo com tamanho dinâmico.
%   \end{enumerate}
%   \item (vide subitens)
%     \begin{enumerate}[label*=\arabic*.]
%       \item fazer um gráfico com as curvas de crescimento de gasto de todas as funções nos Smart Contracts cujo custo aumenta conforme a entrada. Demonstrar que a curva associada à publicação da cifra é a mais acentuada. Informar também ao leitor quais funções tem um comportamento constante em relação à entrada para justificar a sua ausência no gráfico.
%       \begin{enumerate}[label*=\arabic*.]
%         \item referenciar o código no SmartDCPABEFiles e explicar que os dados relacionados a um documento na blockchain excedem a capacidade da pilha da EVM, sendo necessário dividir as informações em duas partes, e uma destas partes contém metadados do arquivo, enquanto a outra parte contém somente a cifra. Explicar que uma alteração na cifra não leva à alteração completa do registro do arquivo, mas somente a uma substituição daquela parte que representa a cifra, no Smart Contract.
%         \item listar em uma tabela (ou em um gráfico de barras) o custo de publicação para cada uma das regras descritas subseção \ref{sec:sub:padronizacao-permissoes}.
%       \end{enumerate}
%       \item mostrar um gráfico de linha com a cotação do Ether dos últimos 12 meses. Citar a alta histórica de dezembro de 2017 e descobrir qual foi a maior variação da cotação do ethereum em um único dia (citar fonte).
%       \item não sei o que informar nessa hipótese pra validar a afirmação dela. Talvez eu não desenvolva um texto sobre isso.
%       \item explicar que a construção criptográfica da DCPABE, até o momento, não admite a revocação de chaves pessoais de atributos de um determinado usuário.
%       \begin{enumerate}[label*=\arabic*.]
%         \item Descrever todos os fatores que podem influenciar nos custos (em tempo e Ether) de substituição de um atributo por outro.
%         \item fazer um gráfico em barras, onde cada barra representa uma atividade do usuário (consultar usuário, consultar certificador, emitir atributo, publicar arquivo, etc) usando um cenário de uso mínimo (descrever este ``uso mínimo'' como a criação de somente um atributo, criptografia e publicação de um arquivo da pessoa A, requisição do atributo de uma pessoa B e obtenção do arquivo publicado por A). Mensurar a diferença de execução dos contratos com e sem verificação e colocar essa diferença em um gráfico de barras (Obs: leva pouquíssimo tempo para adequar as funções a esse tipo de verificação, então é viável mensurar essas diferenças).
%       \end{enumerate}
%   \end{enumerate}
% \end{enumerate}
% % FIM DOS DADOS E ANÁLISES
...
% \subsubsection{Inicialização do sistema}

% O módulo tem os seguintes passos.

% Instanciação de variáveis globais.
% Carregamento de dados de cache

% \subsubsection{Criação de Usuário e de atributos}
...
% \subsubsection{Requisição e concessão de atributos a usuários}
...
% \subsubsection{Operações criptográficas com ABE}
...
% \subsubsection{Envio e recuperação de arquivos criptografados do servidor}
...
% \subsubsection{Publicação e consulta de arquivos na Blockchain}
...
Uma cifra ABE é formada por um grupo de quatro vetores de bytes, conforme demonstra a subseção \ref{sec:sub:criptografia-abe}.
...
A figura X apresenta o crescimento do tamanho da cifra em relação ao número de atributos utilizados na política de acesso.
Para obter estes dados, a biblioteca DCPABE foi utilizada para gerar cifras a partir de políticas de acesso aleatórias ($n=10$), contendo de 1 até 100 atributos.

Figura X

Repare que o tamanho da cifra cresce linearmente em relação a quantidade de atributos. 

%\item Cifras (ciphertexts) ABE podem ser colocadas dentro de transações na Blockchain, mas até um certo limite devido à estrutura do Protocolo Ethereum.
%   \begin{enumerate}[label*=\arabic*.]
  %     \item Isto porque o limite de uso de gas em um bloco acaba impondo limites ao tamanho das transações e, por decorrência, ao tamanho da Cifra.
  %     \begin{enumerate}[label*=\arabic*.]
    %       \item Logo, Podemos calcular o tamanho máximo da cifra de acordo com o limite de gás imposto ao bloco.
    %       \begin{enumerate}[label*=\arabic*.]
      %         \item Entretanto, o limite máximo da cifra mensurado em testes será inferior ao calculado pelo limite de gás, porque a conta não considera o custo de gas das outras instruções executadas pela função que armazena a cifra ou pela função que recupera a cifra.
      %       \end{enumerate}
      %       \item Não há outros fatores que limitem o tamanho da cifra na Blockchain além do limite de gas usado.
      %       \item O custo da transação de publicação da cifra terá um crescimento não-linear (superior a linear) em relação ao seu tamanho em bytes, porque a rede Ethereum impõe que custo de armazenamento permanente de variáveis de tamanho dinâmico aumente em relação ao seu tamanho.
      %     \end{enumerate}
      %   \end{enumerate}
      ...
      %   \begin{enumerate}[label*=\arabic*.]
        %     \item (vide subitens)
        %     \begin{enumerate}[label*=\arabic*.]
          %       \item \label{enum:custo-teorico-de-gas-ciphertext} Descobrir o limite de gás atual e o histórico de limites. Calcular a partir disso o tamanho máximo da cifra. Mostrar em um gráfico o histórico do limite de gás ou, ao invés do gráfico, mostrar a fórmula que influencia o cálculo do limite de gás ao longo do tempo no Ethereum.
          %       \begin{enumerate}[label*=\arabic*.]
%         \item \label{enum:custo-de-gas-ciphertext} Mensurar o gasto de gas das transações de armazenamento e recuperação de ciphertexts da blockchain. Mostrar um gráfico com o crescimento do gasto de gas de acordo com um aumento do tamanho do ciphertext (ou seja, do aumento da política de acesso), até o limite da falha (i.e, até as transações irem acima do limite de gas do Ethereum). Obter o gasto de gas das transações pela diferença entre o custo de uma transação com um ciphertext em seu tamanho teórico máximo e o tamanho máximo em que ele chegou nos testes.
%       \end{enumerate}
%       \item não sei bem como demonstrar que o limite de gas é a única limitação ao tamanho da cifra.
%       \item usar os dados de \ref{enum:custo-de-gas-ciphertext} para demonstrar que o crescimento do gasto de gas não é linear. Explicar a razão disso citando a definição do protocolo Ethereum e do custo associado ao armazenamento de variáveis de tamanho variável.
%     \end{enumerate}
%   \end{enumerate}
