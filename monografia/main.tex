...
Certificadores são os atores do sistema responsáveis pela criação, publicação e concessão de atributos criados aos usuários que necessitem deles (e tenham, claro, o direito de solicitá-los).
Os certificadores usam um Smart Contract para disponibilizar os atributos criados por eles aos usuários.
Os usuários consultam por um endereço de carteira Ethereum neste mesmo Smart Contract e obtêm uma lista dos atributos publicados por meio do endereço consultado, que pode ser inclusive uma lista vazia, no caso de ainda não existirem atributos publicados por aquele endereço.
...
O sistema proposto trabalha com um cenário sem impedimentos para o registro de novos certificadores no sistema por se tratar de uma prova de conceito.
Em um cenário de produção, seria prudente aplicar restrições ao cadastro de novas instituições, adicionando por exemplo uma lista de endereços de administradores do sistema.
...
Uma outra solução, menos restritiva e igualmente viável, é continuar permitindo que as instituições se cadastrem, mas exigir a apresentação de uma mensagem assinada por algum dos endereços de administração \cite{Marx2018}, contendo na mensagem um hash de um dado exclusivo daquela instituição, como o nome ou endereço de carteira dela.
Nesse cenário, a instituição primeiro procuraria a administração do sistema e receberia dela uma assinatura e a informação do endereço assinante.
Depois disso, a instituição encaminharia seus dados ao Smart Contract juntamente com a assinatura, que seria verificada pelo Smart Contract para confirmar que a assinatura é válida e que o endereço assinante também consta na lista de administradores.
