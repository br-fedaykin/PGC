...
Certificadores são os atores do sistema responsáveis pela criação e publicação de atributos, bem como a concessão de atributos sob sua posse aos usuários que necessitem delas e tenham, claro, o direito de solicitá-las.
Os certificadores usam um Smart Contract para disponibilizar os atributos criados por eles aos usuários.
Os usuários consultam por um endereço de carteira Ethereum neste mesmo Smart Contract e obtêm uma lista de atributos publicados em transações usando o endereço consultado, que pode ser uma lista vazia se ainda não existirem atributos publicados por aquele endereço.
Desta forma, o endereço de carteira atua como a identificação primária de uma instituição certificadora e os atributos são reconhecidos como propriedade do dono do endereço da carteira responsável pelas suas publicações.
A fim de permitir a identificação das instituições sem a necessidade de fontes de consulta externas à Blockchain, um Smart Contract próprio para a gestão de certificadores cataloga estas instituições, associando os endereços de carteira a um nome e contato de e-mail. % (ok) TODO qual operação?
