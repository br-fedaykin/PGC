...
Adicionando tags do tipo ``\emph{acesso=restrito}'' ao exemplo anterior pode-se pensar em um cenário onde o usuário precise concordar previamente com termos de compromisso definidos pela administração do gerador de chaves para então obter a chave privada que combine com uma identidade na forma ``\emph{Bob@mail.com ano-2020 acesso=restrito}''.
...
As estruturas de acesso descritas anteriormente foram inicialmente construídas e distribuídas como parte das chaves privadas conferidas aos usuários, enquanto que o conjunto de atributos necessários à descriptografia era informado na cifra. % TODO: padronizar: ou usar cifra/descrifra ou encripta/descripta. Ver em todos os lugares do texto
...
Para impedir ataques de colusão por parte dos usuários a geração das chaves é feita usando um identificador global \emph{(GID)} de usuário, que é utilizado para "vincular" os atributos de diferentes autoridades a um mesmo usuário. % TODO: padronizar e diferenciar chave dee identificador no texto todo.
...
Com a DCPABE pode-se finalmente integrar com segurança bases de dados distintas sob controle de entidades separadas. % TODO: bases de dados de que? explicar melhor
...
O módulo Blockchain é responsável por armazenar de forma permanente e imutável {\color{blue}os metadados} dos documentos cifrados e suas respectivas políticas de acesso. Para impor verificações de acesso e validade de uma transação (seja uma inserção ou modificação de um dado), foi necessário adotar os Smart Contracts. %, descrita em termos de uma forma booleana composta por funções lógicas E e OU com atributos como predicados.
Nesse sentido, foi escolhida a rede Ethereum por suportar nativamente a execução de Smart Contracts e porque é um projeto bem-sucedido entre as opções existentes, considerando tanto o tamanho da comunidade envolvida no desenvolvimento quanto sua posição de mercado. %A Blockchain na qual se baseia o sistema é uma versão privada da rede Ethereum gerada e processada por um programa específico para este fim, denominado Ganache\cite{ganache}.

Para fornecer conectividade à Blockchain Ethereum foi utilizada a biblioteca Web3j.
Por meio dela, o módulo Cliente pode conectar-se à Blockchain principal ou à de teste, sendo esta última utilizada para emular a execução da Blockchain principal. %A biblioteca também possui funcionalidades para gerar classes geradas dinamicamente baseadas no código de Smart Contracts, facilitando a integração do Ethereum com projetos Java.
...
(2) cabe à autoridade usar o módulo Cliente para aplicar as políticas de acesso nos PEPs - Prontuários Eletrônicos de Pacientes; %, tarefa que pode ser automatizada no caso de uma migração de bases de dados para o sistema proposto;
...
(4) durante o envio de um arquivo ao servidor, cabe ao  módulo Cliente acionar um Smart Contract de gestão de arquivos cifrados e cadastrar os metadados referentes ao arquivo, entre eles o servidor de hospedagem e o código de acesso; % TODO: existe código de acesso, identificador, ficou confuso. Explicar melhor esses conceitos ou padronizar.
...
\[paciente \vee (Hospital-X \wedge Hematologista)\] % TODO: inserir um nome de um hospital (verificar todas as ocorrências de X)

Essa política garante acesso ao próprio paciente e aos médicos da especialidade Hematologista que estejam vinculados ao Hospital-X. A política pode ser descrita trocando os símbolos dos operadores lógicos pelo seus nomes em inglês, ou seja, ``paciente OR (Hospital-X AND Hematologista)''. % TODO: a qualquer paciente? Explicar brevemente como o sistema sabe que é ela (Alice) e não qualquer paciente
...
A identidade principal do certificador é seu endereço de carteira ao invés de seu nome, acrescentada ao sistema somente para facilitar a operação e prover um sistema de identificação básico na Blockchain sem a necessidade de terceiros. %TODO qual operação?
...
Isso resultou em uma segunda taxonomia, disponível no Anexo \ref{etc:anexoI}, que serve como referência para a criação de atributos específicos para profissionais com escopos comuns entre os diversos certificadores. % TODO: no pdf diz Anexo 7 (está errado, deveria dizer Anexo I). Ver como criar anexo (não é como section).
...
A DCPABE fornece todas as ferramentas necessárias para o uso do esquema ABE, incluindo a geração de parâmetros globais, geração de chaves privadas, públicas e pessoais dos atributos, cifra e deciframento de fluxos de dados. % TODO: manter (padronizar) a mesma palavra (decifrar, decriptar, descriptografar) no texto todo.
...
Esse esquema permite que uma entidade possa ser identificável de forma única sem a necessidade de controle centralizada sobre o cadastro de novas unidades. % TODO: unidades? que unidades? se for instituição ou entidade padronizar no texto todo
